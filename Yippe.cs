...
namespace FlyingKarlsonMod
{
    public class Yippe : MelonMod
    {
		private GameObject mainPlayer;
		private bool playing;
        private bool done;

		public float TimeModification = 1f;
		...
		public override void OnUpdate()
        {
            ...
            if (Input.GetKeyDown(KeyCode.N))
            {
                ...
                this.StartGame();
                LoggerInstance.Msg("Skipped!");
            }

            if (Input.GetKeyDown(KeyCode.P))
            {
                LoggerInstance.Msg("Restarting...");
                    SceneManager.LoadScene(SceneManager.GetActiveScene().name);
                    Time.timeScale = 1f;
                    this.StartGame();
                LoggerInstance.Msg("Restarted!");
            }
            ...
            if (1 == 1)
            {
				...
				{
					TimeModification = TimeModification + 0.05f;
					LoggerInstance.Msg("TimeScale set to " + TimeModification);
				}
			...
			//               }
			//
			// Rigidbody rb = mainPlayer.GetComponent<Rigidbody>();
			//if (rb == null)
			//               {
			// LoggerInstance.Msg("Rigidbody not found on mainPlayer!");
			// return;
